...
$res_count = mysqli_query($link, "SELECT COUNT(*) FROM `товары` WHERE `Код категории`='1'  ORDER BY  $sorting");
...
for ($i = 1; $i <= $str_page; $i++)
{
    ...
    <ul class="hr">
    <li>
    <a href = Bouquets.php?page='.$i.'>'.$i.'</a>
    </li>
    </ul>';
}
?>
...
$result = mysqli_query($link,"SELECT * FROM `товары` WHERE `Код категории`='1'  ORDER BY  $sorting LIMIT $number, $limit");
