...
if ($_SESSION['auth_admin'] == "yes_auth")
{
    ...
    if (isset($action))
    {
        ...
        {
            case 'delete':
                $delete = mysqli_query($link, "DELETE FROM `товары` WHERE `Код т`='$id'");
            break;
        }
    }
?> 
...
<?php
    ...
    $all_count = mysqli_query($link, "SELECT * FROM `товары`");
...
$res_count = mysqli_query($link, "SELECT COUNT(*) FROM `товары` $cat");
...
$result = mysqli_query($link,"SELECT * FROM `товары` $cat ORDER BY  `Код т` ASC LIMIT $number, $limit");
...
do
{
    ...
    echo '
        ...
        <a class="green" href="edit_product.php?id='.$myrow["Код т"].'">Изменить</a> | <a rel="tovar.php?'.$url.'id='.$myrow["Код т"].'&action=delete" class="delete">Удалить</a>
        </p>
        </li>
        ';
